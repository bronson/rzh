...
Tells rzh to wait for a zmodem transfer to be initiated, handle
the transfer, and
exit when it has finished.  If a single argument is passed on
the command line, it is interpreted as the directory to contain
the transferred files.  If no argument is supplied, files are
saved to the current directory.

=item B<-s> B<--send>

Immediately sends the files named on the command line and exits.
NOTE: this appears somewhat buggy.  Use sz instead.
